...
def OF_t95(demand, demand_nodes, expected, dtm, threshold=95,):

    # Compute system functionality (%)

    Qi_total = demand[demand_nodes].sum(axis=1)
    DQi_total = expected.sum(axis=1)

    functionality = (
        100 * Qi_total / DQi_total
    ).clip(lower=0, upper=100)

    values = functionality.to_numpy()
    simulation_time = functionality.index.to_numpy()

    # Search for the first permanent recovery
    for i in range(len(values)):

        permanently_recovered = (
            values[i] >= threshold
            and np.all(values[i:] >= threshold)
        )

        if permanently_recovered:
            return simulation_time[i] / 60      # minutes

    # Stable recovery not achieved
    return simulation_time[-1] / 60
